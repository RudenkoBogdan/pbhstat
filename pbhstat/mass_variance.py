...
class MassVariance:
    ...
    def evaluate(self, k_values):
        ...
        for i in range(len(r)):
            ...
            sigma["sigma0"][i] = pref * np.trapezoid(Wkr2 * kr**4 * Pk, lnk)
            sigma["sigma1peaks"][i] = pref * np.trapezoid(Wkr2 * kr**4 * k**2 * Pk, lnk)
            sigma["sigma1nl"][i] = pref * np.trapezoid(Wkr2 * kr**6 * Pk, lnk)
            sigma["sigma2"][i] = pref * np.trapezoid(Wkr2 * kr**8 * Pk, lnk)
            sigma["sigmaw"][i] = pref * np.trapezoid((Wkr * kr**4 - 2 * kr**2 * Wkr)**2 * Pk, lnk)
            sigma["sigmav"][i] = pref * np.trapezoid((3 * kr * np.sin(kr) - kr**2 * Wkr)**2 * Pk, lnk)
            sigma["sigmavg"][i] = pref * np.trapezoid((3 * kr * np.sin(kr) - kr**2 * Wkr) * Wkr * kr**2 * Pk, lnk)
            sigma["sigmagw"][i] = pref * np.trapezoid((kr**6 - 2 * kr**4) * Wkr2 * Pk, lnk)
            sigma["sigmavw"][i] = pref * np.trapezoid((3 * kr * np.sin(kr) - kr**2 * Wkr) * (Wkr * kr**4 - 2 * Wkr * kr**2) * Pk, lnk)
